...
import { ColapceComponent } from './colapseComponent';
...
const CVComponent: FC<CVCompProps> = ({ currentCV }) => {
	...
	useEffect(() => {
		if (selectedTags.length > 0)
			setExpiriense(
				currentCV.expiriense.filter((item) => item.tags.filter((x) => selectedTags.includes(x)).length > 0)
			);
		else setExpiriense(currentCV.expiriense);
	}, [selectedTags]);
		...
		<div id="app" className="app">
			...
			<div className="cvTitle">
				<div className="contactInfo">
					<ContactInformationItem style="contactPhone" title={currentCV.local.Phone}>
						{currentCV.contactInfo.phone}
					</ContactInformationItem>
					<ContactInformationItem style="contactEmail" title={currentCV.local.Email}>
						{currentCV.contactInfo.email}
					</ContactInformationItem>
					<ContactInformationItem style="contactLinkedin" title={currentCV.local.LinkedIn}>
						{!currentCV.contactInfo.linkedIn ? (
							''
						) : (
							<Link href={currentCV.contactInfo.linkedIn || ''} target="_blank">
								{currentCV.local.profile}
							</Link>
						)}
					</ContactInformationItem>
					<ContactInformationItem style="contactWeb" title={currentCV.local.Web}>
						{currentCV.contactInfo.website}
					</ContactInformationItem>
					<ContactInformationItem style="contactAddress" title={currentCV.local.Address}>
						{currentCV.contactInfo.address}
					</ContactInformationItem>
				</div>
			...
			<div className="descriptionHolder">
				{currentCV.description?.map((line, index) => (
			...
			<div className="content">
				<div className="contentRow">
					<div className="contentCol colHead">
						<h2 className="colHead">{currentCV.local.Languages}</h2>
					</div>
				...
				<div className="contentRow">
					<div className="contentCol colHead">
						<h2 className="colHead">{currentCV.local.Education}</h2>
					</div>
					<div className="contentCol colBody">
				...
				<ColapceComponent title={currentCV.local.Experience}>
					<div className="contentRow">
						<div className="contentCol colHead">
						...
						<div className="contentCol colBody">
							{expiriense
								.sort((a: Expiriense, b: Expiriense) =>
				...
				<ColapceComponent title={currentCV.local.Projects} deffold>
					<div className="contentRow">
						<div className="contentCol colHead">
							<h2 className="colHead">{currentCV.local.Projects}</h2>
						</div>
						<div className="contentCol colBody">
							{currentCV.projects
								.filter((itm) => itm.link)
